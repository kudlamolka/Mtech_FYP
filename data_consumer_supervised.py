#!/usr/bin/env python3
"""
Data Consumer (Supervised) - Receives streaming data via socket and detects anomalies using Random Forest.
"""
...
def start_consumer():
    """
    Connect to producer, receive data, and detect anomalies using Random Forest.
    """
    ...
    print("Loading supervised model artifacts...")
    rf_model = joblib.load('models/supervised_rf_model.pkl')
    scaler = joblib.load('models/supervised_scaler.pkl')
    print(f"Random Forest model loaded. Anomaly probability threshold: {threshold}")
    
    feature_engine = StreamingFeatureEngine()
    
    os.makedirs('logs', exist_ok=True)
    log_file = f"logs/streaming_anomalies_supervised_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    anomaly_log = []
        ...
        while '\n' in buffer:
            ...
            try:
                ...
                if features is not None:
                    ...
                    anomaly_proba = rf_model.predict_proba(feature_scaled)[0, 1]  # Probability of class 1 (anomaly)
                    ...
                    if is_anomaly:
                        ...
                        log_entry = {
                            ...
                            'price': close,
                            'volume': volume,
                            'anomaly_probability': anomaly_proba,
                            'prediction': int(prediction),
                            **features
                        }
                        anomaly_log.append(log_entry)
                        print(f"ANOMALY: {stock} | {timestamp} | Price: ₹{close:.2f} | Prob: {anomaly_proba:.4f}")

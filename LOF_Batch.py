...
def process_anomalies_with_optimal_threshold(df, tracker_path):
    ...
    # LOF needs to be fit on the data
    lof.fit(X_scaled)
    
    print("Extracting continuous anomaly scores...")
    # LOF uses negative_outlier_factor_ (more negative = more anomalous)
    df['raw_anomaly_score'] = lof.negative_outlier_factor_
    ...
    # LOF: more negative = more anomalous, so we use negative of the score
    ...
    joblib.dump(optimal_score_threshold, 'models/lof_optimal_threshold.pkl')
    print(f"\n>>> LOF model artifacts saved to models/ directory <<<")
...
if __name__ == "__main__":
    ...
    output_file = "minute_stock_anomalies_detected_lof.csv"
